...
public class Graph {
    ...
    public Graph prims(int start) {
        ...
        HashMap<Integer, Integer> distFromTree = new HashMap<>();
        int[] prevVertex = new int[getAllVertices().size()];
        ArrayList<Integer> visited = new ArrayList<>();
        PriorityQueue<Integer> fringe = new
                PriorityQueue<>(edges.size(), (a, b) -> distFromTree.get(a) - distFromTree.get(b));

        for (Edge e : edges.get(start)) {
            distFromTree.put(e.getDest(), e.getWeight());
            prevVertex[e.getDest()] = start;
            fringe.add(e.getDest());
        }
        visited.add(start);
        spanTree.addVertex(start);

        while (!fringe.isEmpty() && spanTree.getAllEdges().size()
                < getAllVertices().size()) {
            int v = fringe.poll();
            if (!visited.contains(v)) {
                spanTree.addEdge(prevVertex[v], v, distFromTree.get(v));
                for (Edge e : getEdges(v)) {
                    if (distFromTree.containsKey(e.getDest())) {
                        if (distFromTree.get(e.getDest()) > e.getWeight()) {
                            distFromTree.put(e.getDest(), e.getWeight());
                            prevVertex[e.getDest()] = e.getSource();
                        }
    ...
    public Graph kruskals() {
        Graph spanTree = new Graph();
        WeightedQuickUnionUF union = new WeightedQuickUnionUF(getAllVertices().size());
        ArrayList<Edge> sortedEdges = new ArrayList<>();

        for (int v: neighbors.keySet()) {
            spanTree.addVertex(v);
        }
        sortedEdges.addAll(getAllEdges());

        if (!sortedEdges.isEmpty()) {
            for (Edge e : sortedEdges) {
                if (spanTree.getAllEdges().size() < getAllVertices().size()) {
                    if (!union.connected(e.getSource(), e.getDest())) {
                        spanTree.addEdge(e);
                        union.union(e.getSource(), e.getDest());
                    }
                }
            }
        }
    ...
    public boolean isConnected(int u, int v) {
        if (getNeighbors(u).isEmpty() || getNeighbors(v).isEmpty()) {
            return false;
        }
        if (isNeighbor(u, v)) {
            return true;
