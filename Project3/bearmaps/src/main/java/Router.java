...
public class Router {
    ...
    public static List<Long> shortestPath(GraphDB g, double stlon,
        ...
        HashMap<Long, Long> prevVertex = new HashMap<>();
        Comparator<Long> priorityComparator = Comparator.comparingDouble((o ->
                ((distFromStart.get(o) + g.distance(o, last)))));
        PriorityQueue<Long> fringe = new PriorityQueue<>(g.numVertices(), priorityComparator);
        ...
        while (!fringe.isEmpty()) {
            long vertex = fringe.poll();
            if (vertex == end) {
                while (prevVertex.get(vertex) != vertex) {
                    shortestPath.addFirst(vertex);
                    vertex = prevVertex.get(vertex);
                }
            ...
            if (!marked.contains(vertex)) {
                for (Edge e : g.getEdges(vertex)) {
                    if (!distFromStart.containsKey(e.getTo())) {
                        distFromStart.put(e.getTo(), e.getDistance() + distFromStart.get(vertex));
                        prevVertex.put(e.getTo(), vertex);
                        fringe.add(e.getTo());
                    } else {
                        if (distFromStart.get(e.getTo())
                                > distFromStart.get(vertex) + e.getDistance()) {
                            distFromStart.replace(e.getTo(),
                                    distFromStart.get(vertex) + e.getDistance());
                            prevVertex.replace(e.getTo(), vertex);
                            fringe.add(e.getTo());
                        }
                    }
                }
