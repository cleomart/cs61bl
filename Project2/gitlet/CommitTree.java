...
public class CommitTree implements Serializable {
    ...
    public void merge(String givenBranchName) {
        ...
        for (String nameOfSplit: blobsSplitPoint.keySet()) {
            ...
            if (blobsCurr.containsKey(nameOfSplit) && blobsGiven.containsKey(nameOfSplit)) {
             ...
             if (!blobsCurr.containsKey(nameOfSplit)
                        && blobsGiven.containsKey(nameOfSplit) && !blobsGiven.containsValue(idOfSplit)) {
                    /*String givenID = blobsGiven.get(nameOfSplit);
    ...
    public void mergeRewrite(String fileName, String givenID, String currID) {
        ...
        String allContent;
        if (givenID == null) {
            currFile = new File(".gitlet/blobs/" + currID);
            contentOfCurr = Utils.readContents(currFile);
            allContent = head + new String(contentOfCurr) + middle + tail;
        } else if (currID == null) {
            givenFile = new File(".gitlet/blobs/" + givenID);
            contentOfGiven = Utils.readContents(givenFile);
            allContent = head + middle + new String(contentOfGiven) + tail;
        } else {
            currFile = new File(".gitlet/blobs/" + currID);
            givenFile = new File(".gitlet/blobs/" + givenID);
            contentOfCurr = Utils.readContents(currFile);
            contentOfGiven = Utils.readContents(givenFile);
            allContent = head + new String(contentOfCurr) + middle + new String(contentOfGiven)+ tail;
        }
        Utils.writeContents(rewrite, allContent.getBytes());
    }
